	...
	$: {
		...
		if (content.includes(thinkStartTag)) {
			...
			if (thinkEndIndex > thinkStartIndex) {
				thinkingContent = content.substring(thinkStartIndex + thinkStartTag.length, thinkEndIndex);
				finalContent = content.substring(thinkEndIndex + thinkEndTag.length);
				thinkCompleted = true;
			} else {
				thinkingContent = content.substring(thinkStartIndex + thinkStartTag.length);
				finalContent = '';
				thinkCompleted = true;
			}
		} else {
		...
		if (message.content) {
			...
			if (currentLength > lastContentLength + 50) {
				...
				tick().then(() => {
					if (messageElement && isLastMessage) {
						const headerHeight = 60;
						const targetPosition = messageElement.getBoundingClientRect().top + window.pageYOffset - headerHeight - 120;
						window.scrollTo({
